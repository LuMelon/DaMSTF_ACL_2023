...
class ImageClassifier(VirtualModel, nn.Module):
    ...
    def AdvDLossAndAcc(self, discriminator:nn.Module, batch:OfficeHomeBatch, grad_reverse=False):
        f = self.Batch2Vecs(batch)
        f1 = self.bottleneck(f)
        if grad_reverse:
            f1 = GradientReversal.apply(f1)
        predictions = discriminator(self.dropout(f1)).softmax(dim=1)
...
def RunTask(source=["Cl"], target="Pr", fewShotCnt=20, data_dir='/mnt/VMSTORE/OfficeHome/', model_path='../../resnet50-19c8e357.pth'):
    ...
    trainer = DANNTrainer(random_seed=10086, log_dir=logDir, suffix=f"{newDomainName}_FS{fewShotCnt}",
                            model_file=f"./DANN_{newDomainName}_FS{fewShotCnt}.pkl",
