...
from BaseModel.AmazonModel import Senti_BERT, DomainDiscriminator
from DomainAdaptationTrainer.MME import MMETrainer
import numpy as np
from TrainingEnv import GradientReversal

class MME_Amazon_Model(Senti_BERT):
...
def obtain_model(bertPath, model_device, lr_model):
    ...
    # Create the model
    bert = MME_Amazon_Model.from_pretrained(bertPath, config=bert_config).to(model_device)
...
def RunTask(tID, fewShotCnt, logDir, bert_path, database_name="./Sentiment.db"):
    ...
    trainer = MMETrainer(seed=10086, log_dir=logDir, suffix=f"{target_domain_name}_FS{fewShotCnt}",
                            model_file=f"./MME_{target_domain_name}_FS{fewShotCnt}.pkl",
